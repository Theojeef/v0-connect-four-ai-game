...
import type { AIThought } from "../lib/minimax-ai"
...
export function useAIWorker() {
  ...
  const requestCallbacks = useRef<Map<string, (result: AIWorkerOutput) => void>>(new Map())
      ...
      try {
        ...
        const workerCode = `
          ...
          self.addEventListener("message", (event) => {
            ...
            const { board, difficulty, player, requestId } = event.data;
          ...
          function getAIMove(board, difficulty, player = 2) {
          ...
          function getOpeningBookMove(board, player) {
          ...
          function evaluateKeyPositions(board, player) {
            let score = 0;
            const opponent = player === 1 ? 2 : 1;
        ...
        worker.addEventListener("message", (event) => {
          const result = event.data as AIWorkerOutput

          // Call the registered callback for this request
          ...
          // If this is the active request, update the state
          if (result.requestId === activeRequestId.current) {
            setLastResult({
        ...
        // Fallback to non-worker implementation could be added here
      }
    }
  }, [])
  ...
  const calculateMove = useCallback(
    (
      ...
      difficulty: "easy" | "medium" | "hard",
      player: 1 | 2 = 2,
    ): Promise<{ column: number; thoughts: AIThought[]; timeElapsed: number }> => {
      return new Promise((resolve, reject) => {
        if (!workerRef.current) {
          // Fallback to direct calculation if worker isn't available
          ...
          // If no winning move, prefer center column
          if (selectedCol === -1) {
            if (board[0][3] === 0) {
              selectedCol = 3
            } else {
