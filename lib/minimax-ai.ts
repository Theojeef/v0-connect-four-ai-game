...
export function getAIMove(
  ...
  player: Player = 2,
): { column: number; thoughts: AIThought[] } {
...
function getOpeningBookMove(board: Board, player: Player): number {
...
function evaluateKeyPositions(board: Board, player: Player): number {
  let score = 0
  const opponent = player === 1 ? 2 : 1
